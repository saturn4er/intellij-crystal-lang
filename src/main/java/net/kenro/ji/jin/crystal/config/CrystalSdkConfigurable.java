...
package net.kenro.ji.jin.crystal.config;
...
import net.kenro.ji.jin.crystal.sdk.CrystalSmallIDEsSdkService;
...
public class CrystalSdkConfigurable implements SearchableConfigurable, Configurable.NoScroll {
  ...
  public CrystalSdkConfigurable(@NotNull Project project, boolean dialogMode) {
    myProject = project;
    if (dialogMode) {
  ...
  public void apply() throws ConfigurationException {
    ApplicationManager.getApplication().runWriteAction(() -> {
      ...
      Library get = table.getLibraryByName(CrystalSmallIDEsSdkService.LIBRARY_NAME);
      Library lib = get != null ? get : table.createLibrary(CrystalSmallIDEsSdkService.LIBRARY_NAME);
      ...
      String sdkPath = CrystalSDKUtil.adjustSdkPath(mySdkPathField.getText());
      String versionString = CrystalSDKUtil.retrieveCrystalVersion(sdkPath);
      boolean toRemove = StringUtil.isEmpty(sdkPath) || versionString == null;

      if (!toRemove) {
        for (VirtualFile file : CrystalSDKUtil.getSdkDirectoriesToAttach(sdkPath, versionString)) {
      ...
      CrystalSdkService.getInstance(myProject).incModificationCount();
    });
  }

  @Override
  public void reset() {
    mySdkPathField.setText(StringUtil.notNullize(CrystalSdkService.getInstance(myProject).getSdkHomePath(null)));
  }

  @Override
  public boolean isModified() {
    String currentPath = StringUtil.notNullize(CrystalSdkService.getInstance(myProject).getSdkHomePath(null));
    return !CrystalSDKUtil.adjustSdkPath(mySdkPathField.getText()).equals(currentPath);
  }
  ...
  private void asyncUpdateSdkVersion(@NotNull String sdkPath) {
    ...
    if (!myAlarm.isDisposed()) {
      ...
      myAlarm.addRequest(() -> {
        String version = CrystalSDKUtil.retrieveCrystalVersion(CrystalSDKUtil.adjustSdkPath(sdkPath));
  ...
  private void createUIComponents() {
    ...
    AsyncProcessIcon gettingVersionIcon = new AsyncProcessIcon("Getting Crystal version");
  ...
  private class MyBrowseFolderListener extends ComponentWithBrowseButton.BrowseFolderActionListener<JTextField> {
    public MyBrowseFolderListener(@NotNull FileChooserDescriptor descriptor) {
      super("Select Crystal SDK Path", "", mySdkPathField, myProject, descriptor, TextComponentAccessor.TEXT_FIELD_WHOLE_TEXT);
    }

    @Nullable
    @Override
    protected VirtualFile getInitialFile() {
      return ObjectUtils.chooseNotNull(super.getInitialFile(), CrystalSDKUtil.suggestSdkDirectory());
    }
  }
