...
import net.kenro.ji.jin.crystal.config.CrystalSdkConfigurable;
import net.kenro.ji.jin.crystal.util.CrystalConstants;
import org.jetbrains.annotations.NotNull;
import org.jetbrains.annotations.Nullable;

public class CrystalSmallIDEsSdkService extends CrystalSdkService {
    public static final String LIBRARY_NAME = "Crystal SDK";

    public CrystalSmallIDEsSdkService(@NotNull Project project) {
        super(project);
    }
    ...
    public String getSdkVersion(@Nullable Module module) {
        ...
        return CachedValuesManager.getManager(myProject).getCachedValue(holder, () -> {
            ...
            if (sdkHomePath != null) {
                result = CrystalSDKUtil.retrieveCrystalVersion(sdkHomePath);
            }
    ...
    public void chooseAndSetSdk(@Nullable Module module) {
        ShowSettingsUtil.getInstance().editConfigurable(myProject, new CrystalSdkConfigurable(myProject, true));
    }

    @Nullable
    @Override
    public Configurable createSdkConfigurable() {
        return !myProject.isDefault() ? new CrystalSdkConfigurable(myProject, false) : null;
    }

    @Override
    public boolean isCrystalModule(@Nullable Module module) {
        return super.isCrystalModule(module) && getSdkHomePath(module) != null;
    }

    public static boolean isCrystalSdkLibRoot(@NotNull VirtualFile root) {
        return root.isInLocalFileSystem() &&
                root.isDirectory() &&
                (VfsUtilCore.findRelativeFile("bin/" + CrystalConstants.EXECUTABLE_NAME, root) != null);
    }
}
